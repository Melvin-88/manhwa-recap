...
## Rules
- НЕ перепроєктовує персонажів чи середовища — лише застосовує стиль і генерує.
- НЕ виносить власного вердикту про якість/консистентність зображення — записує `flagged_for_review: false` за замовчуванням для щойно згенерованого кадру; фінальне значення цього поля належить виключно skill Image QA, який запускається одразу після цього.
