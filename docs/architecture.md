...
**`stories/` не потрапляє в git.** Репозиторій зберігає лише story-agnostic архітектуру пайплайну (`skills/`, `scripts/`, `docs/`) — конкретні історії (реєстри, епізоди, згенеровані кадри/аудіо/відео, source-material) це робочі дані конкретної адаптації, а не частина спільного процесу. `stories/` цілком у `.gitignore`; усі файли, описані вище, лишаються локально на диску, просто не версіонуються в git.
